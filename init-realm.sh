...
for r in user admin; do
  curl -s -X POST "$KC/admin/realms/forms-realm/roles" -H "Authorization: Bearer $TOKEN"     -H "Content-Type: application/json" -d "{"name":"$r"}" || true
done

echo "[*] Client forms-spa..."
curl -s -X POST "$KC/admin/realms/forms-realm/clients" -H "Authorization: Bearer $TOKEN"   -H "Content-Type: application/json" -d '{
    "clientId":"forms-spa","publicClient":true,
...
if [ -n "$UID" ]; then
  ...
  curl -s -X POST "$KC/admin/realms/forms-realm/users/$UID/role-mappings/realm"     -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" -d "[$RU]" >/dev/null
fi
echo "[*] Done."
